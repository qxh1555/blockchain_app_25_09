...
const GamePage = () => {
    ...
    useEffect(() => {
        if (user) {
            ...
            socket.on('tradeResult', (result) => {
                ...
                // Show notification for failed trades
                if (!result.success) {
                    setNotification({ 
                        type: 'error', 
        ...
        return (
            <div key={tradeId} className="card mb-3">
                ...
                <div className="card-body">
                    ...
                    {message && (
                        <div className={`alert ${status === 'failed' || status === 'rejected' ? 'alert-danger' : 'alert-info'} mt-2 mb-0 py-2`}>
                            <small>{message}</small>
                        </div>
                    )}
                </div>
    ...
    return (
        <div className="container mt-4">
            {notification && (
                <div className={`alert ${notification.success || notification.type === 'success' ? 'alert-success' : 'alert-danger'} alert-dismissible fade show`}>
