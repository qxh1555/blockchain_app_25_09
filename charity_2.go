...
func (c *CharityCC) InitLedger(ctx contractapi.TransactionContextInterface) error {
    ...
    for _, d := range donations {
        ...
        // 更新 campaign
        campBytes, err := ctx.GetStub().GetState("camp:" + d.CampaignID)
        if err != nil {
            return err
        }
        var camp Campaign
        ...
        camp.Updates = append(camp.Updates, fmt.Sprintf("Donation %s: %d received", d.ID, d.Amount))
        newCampBytes, _ := json.Marshal(camp)
        if err := ctx.GetStub().PutState("camp:"+camp.ID, newCampBytes); err != nil {
            return err
        }

        // 更新用户历史捐赠
        userBytes, err := ctx.GetStub().GetState("user:" + d.DonorID)
        if err != nil {
            return err
        }
        var u User
        ...
        newUserBytes, _ := json.Marshal(u)
        if err := ctx.GetStub().PutState("user:"+u.ID, newUserBytes); err != nil {
            return err
        }
    }
